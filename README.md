...
Spotfire의 IronPython 2.7 스크립팅을 **문법부터 실전 예제까지** 다루는 한국어 교안과,
바로 실행할 수 있는 예제 스크립트 18종입니다.
...
교안은 문법을 먼저 훑고, 그다음 예제를 봅니다.

| 장 | 내용 |
|----|------|
| 1 | 스크립트 실행 환경 — 어디에 넣고, 어떻게 실행하고, 어떻게 디버깅하는지 |
| 2 | IronPython 2.7 문법 — Python 2.7 전반, Python 3와의 차이 |
| 3 | .NET 상호운용 문법 — `clr`, `As[T]()`, 제네릭, .NET 타입 |
| 4 | Spotfire API 객체 모델 — `Document` 아래로 내려가는 지도 |
| 5~8 | 예제 18선 |
| 9 | 실무 팁과 함정 — 성능, Undo, Web Player 호환성 |
| 10 | 치트시트 & FAQ |
...
| 4 | 범례·제목·서식 일괄 통일 | [`04_unify_legend_and_title.py`](scripts/01-bulk/04_unify_legend_and_title.py) |
| 5 | 모든 시각화의 데이터 테이블 일괄 교체 | [`05_swap_data_table.py`](scripts/01-bulk/05_swap_data_table.py) |
| 6 | 모든 차트의 줌·축 범위 초기화 | [`06_reset_zoom_all_charts.py`](scripts/01-bulk/06_reset_zoom_all_charts.py) |
| 7 | 모든 페이지의 시각화를 PNG로 일괄 내보내기 | [`07_export_all_visuals_to_png.py`](scripts/02-data/07_export_all_visuals_to_png.py) |
| 8 | 여러 데이터 테이블을 한 번에 파일로 내보내기 | [`08_export_all_tables_to_file.py`](scripts/02-data/08_export_all_tables_to_file.py) |
| 9 | 마킹한 행을 새 데이터 테이블로 스냅샷 | [`09_snapshot_marked_rows.py`](scripts/02-data/09_snapshot_marked_rows.py) |
| 10 | 마킹 결과를 문서 속성으로 넘기기 | [`10_marking_to_document_property.py`](scripts/02-data/10_marking_to_document_property.py) |
| 11 | 키 컬럼으로 다른 테이블에 마킹 전파 | [`11_propagate_marking_by_key.py`](scripts/02-data/11_propagate_marking_by_key.py) |
| 12 | 모든 데이터 테이블 일괄 새로고침 | [`12_refresh_all_data_tables.py`](scripts/02-data/12_refresh_all_data_tables.py) |
| 13 | 대시보드 전체 상태 초기화 | [`13_reset_dashboard_state.py`](scripts/03-ui/13_reset_dashboard_state.py) |
| 14 | 문서 속성 값으로 페이지 표시/숨김 | [`14_toggle_pages_by_role.py`](scripts/03-ui/14_toggle_pages_by_role.py) |
| 15 | 역할별 필터 패널 구성 | [`15_configure_filter_panel.py`](scripts/03-ui/15_configure_filter_panel.py) |
| 16 | 시각화 유형 일괄 토글 | [`16_bulk_switch_visual_type.py`](scripts/03-ui/16_bulk_switch_visual_type.py) |
| 17 | 마킹한 값별로 시각화 자동 생성 | [`17_generate_visuals_from_marking.py`](scripts/04-advanced/17_generate_visuals_from_marking.py) |
| 18 | 표현식 전수 검사 (문서 감사 리포트) | [`18_audit_expressions.py`](scripts/04-advanced/18_audit_expressions.py) |
