...
!!! danger "검증 포인트 — 보안 장치가 아닙니다"
    ...
    - `Page.Visible` 은 **읽기·쓰기 모두 동작을 확인했습니다.**(14.x)
...
!!! note "검증 포인트"
    - `FilterHandle.Visible`을 `False`로 두면 **필터가 해제되는 것이 아니라 안 보이기만** 합니다.
      값이 걸린 채 숨기면 사용자가 원인을 못 찾으므로, 숨기기 전에 예제 14으로 초기화하세요.
...
전체 초기화([예제 14](10-examples-ui.html))는 애써 맞춰 놓은 기간 설정까지 날려 버립니다.
...
!!! note "검증 포인트"
    - `filteringScheme[dataTable][column]` 이 **핵심 문법**입니다.
      ...
      **동작을 확인했습니다.**(14.x) 반환값은 필터 객체이고 `Reset()` 을 가지고 있습니다.
