...
이 장을 건너뛰면 2장 이후 코드를 어디에 붙여 넣어야 할지 알 수 없습니다.
...
즉 **"Python 2.7 문법으로 .NET API를 호출한다"** 가 전부입니다. 2장에서 앞의 절반을, 3장에서
뒤의 절반을 다룹니다.
...
| 객체 | 타입 | 의미 |
|------|------|------|
| `Document` | `Spotfire.Dxp.Application.Document` | 현재 분석 문서. 페이지·데이터·속성의 최상위 진입점 |
| `Application` | `AnalysisApplication` | 애플리케이션 수준. 서비스 조회, 문서 열기/저장 |
...
!!! tip "예외: 일괄 적용 스크립트"
    5장처럼 "모든 시각화에 적용" 하는 스크립트는 애초에 특정 대상을 지목하지 않으므로
...
| `... is not callable` / `expected X, got Y` | .NET 타입 불일치. 3장 참조 |
| `Column '...' was not found` | 컬럼명 오타 또는 대소문자 불일치(**대소문자를 구분합니다**) |
| `unexpected indent` | 탭과 공백을 섞음. 1.6 참조 |
...
다음 장부터 문법입니다. Python을 이미 아는 분도 **2장의 "Python 3와 다른 점"** 만큼은
반드시 보고 넘어가세요.
