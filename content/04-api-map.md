# 4. Spotfire API 객체 모델

문법을 알아도 **"이 기능이 어느 객체에 매달려 있는지"** 를 모르면 코드를 못 씁니다.
이 장은 `Document`에서 출발해 아래로 내려가는 지도입니다. 5장 이후 예제는 전부 이 지도 위에서 움직입니다.
...
`Visual`(껍데기)과 `VisualContent`(알맹이)의 구분이 핵심입니다 → [3.2 참조](03-dotnet-interop.html#32-ast)
...
"모든 필터 초기화" 버튼은 **현재 스킴 하나만** 초기화합니다. 이것이 [예제 13](07-examples-ui.html)의 출발점입니다.
...
지도는 여기까지입니다. 이제 [5장](05-examples-bulk.html)부터 실제 예제로 들어갑니다.
