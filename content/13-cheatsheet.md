...
`try/except`로 감싸고 `TypeId`로 걸러 내세요 → [3.2 참조](05-dotnet-interop.html#52-ast)
...
반드시 사본에서 먼저 시험하세요 → [9.3 참조](12-tips.html#123-undo)

### Q. Web Player에서 스크립트가 실패합니다

`MessageBox`, 파일 대화상자, 로컬 파일 쓰기는 브라우저에서 동작하지 않습니다.
→ [9.1 참조](12-tips.html#121-analyst-web-player)
