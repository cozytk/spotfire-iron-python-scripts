...
## 예제 7. 모든 페이지의 시각화를 PNG로 일괄 내보내기
...
## 예제 8. 여러 데이터 테이블을 한 번에 파일로 내보내기
...
!!! note "검증 포인트"
    ...
    - `writer.Write`가 요구하는 컬럼 이름은 **파이썬 리스트로 넘어갑니다.**
      타입 오류가 나면 `Array[String](columnNames)`로 변환해 보세요 → [3.5 참조](03-dotnet-interop.html#35-net)
...
!!! note "검증 포인트"
    ...
    - 마킹된 행이 수만 건이면 `values not in values` 검사가 느립니다.
      큰 데이터에서는 `set()`을 쓰세요.
      ...
      for row in sourceTable.GetRows(markedRows, cursor):
          if cursor.CurrentValue is not None:
              seen.add(cursor.CurrentValue)
      values = sorted(seen)
      ```

---

## 예제 11. 키 컬럼으로 다른 테이블에 마킹 전파
